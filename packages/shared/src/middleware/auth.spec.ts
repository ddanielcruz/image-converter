...
describe('auth', () => {
  ...
  it('throws when token is invalid', async () => {
    const { sut } = makeSut()
    await request(sut).get('/').set('x-access-token', 'invalid-token').expect(401)
  })
  ...
  it('throws when token is expired', async () => {
    ...
    await request(sut).get('/').set('x-access-token', token).expect(401)
  })
  ...
  it('sets session and returns 204 on success', async () => {
    ...
    await request(sut).get('/').set('x-access-token', token).expect(204)
  })
})
